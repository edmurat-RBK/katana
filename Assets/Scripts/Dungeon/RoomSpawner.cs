...
public class RoomSpawner : MonoBehaviour
{
    ...
    void CreateSpawnpointAround()
    {
        if (!spawned)
        {
            if (northDoor)
            {
                GameObject newInstance = Instantiate(template.spawnpointPrefab, new Vector3(transform.position.x, transform.position.y + 15f, 0f), Quaternion.identity);
            ...
            if (eastDoor)
            {
                GameObject newInstance = Instantiate(template.spawnpointPrefab, new Vector3(transform.position.x + 22f, transform.position.y, 0f), Quaternion.identity);
                newInstance.GetComponent<RoomSpawner>().roomDistance = roomDistance + 1;
